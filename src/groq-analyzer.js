...
${
  analysis.functions.length > 0 && className
    ? `
ADDITIONAL INSTRUCTIONS FOR STANDALONE FUNCTIONS:
This file contains both class methods AND standalone functions that need testing.

11. Generate tests for ALL functions in the file:
    - Test the "${className}" class methods
    - Test the standalone "${functionNames}" function(s)
12. Create separate describe blocks for class methods and standalone functions
13. Import both the class and standalone functions as shown in the template below
`
    : ''
}
...
${
  analysis.functions.length > 0 && className
    ? `
describe('Standalone ${functionNames} function', () => {
  test('should multiply two numbers', () => {
    const result = ${functionNames}(2, 3);
    expect(result).toBe(6);
  });

  test('should handle edge case of zero', () => {
    const result = ${functionNames}(0, 5);
