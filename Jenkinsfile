pipeline {
    ...
    steps {
        script {
            ...
            def testFiles = sh(
                script: "find __tests__ -name '*.test.js'",
                returnStdout: true
            ).trim().split('\n').findAll { it.trim() }
            ...
            if (testFiles.size() > 0) {
                ...
                sh """
                    git commit -m "🤖 AI-generated unit tests [skip ci]" || echo "No changes to commit"
                    git push https://${GITHUB_TOKEN}@github.com/HarshitMalik22/AIUnitTest.git HEAD:${env.CURRENT_BRANCH}
                """

                echo "✅ ${testFiles.size()} test file(s) pushed."
